...
import CommanClasses.CustomerCollection;
import CommanClasses.Item;
...
public class ShowItems extends JPanel {
	...
	public ShowItems(ObjectInputStream input,ObjectOutputStream output) throws Exception {
		...
		while(i.hasNext())
		{
			item=i.next();
			Barray=item.b;
			File file=new File("E:\\onlineauctionsystem\\admin2\\AdminImages\\"+String.valueOf(item.ID)+".txt");
			fos=new FileOutputStream(file);
			fos.write(Barray);
			image=new ImageIcon("E:\\onlineauctionsystem\\admin2\\AdminImages\\"+String.valueOf(item.ID)+".txt");
